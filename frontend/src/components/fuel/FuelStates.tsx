...
export const FuelEmptyState: React.FC<FuelEmptyStateProps> = ({
  ...
  description = 'Add your first vehicle fuel refueling record to log operational costs.',
  action,
}) => {
  return (
    <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-border bg-card/50 my-6">
      <div className="p-4 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 mb-4">
...
export const FuelErrorState: React.FC<{ title?: string; description?: string; onRetry?: () => void }> = ({
  title = 'Failed to load fuel records',
  description = 'An error occurred while connecting to the telemetry service.',
  onRetry,
}) => {
  return (
