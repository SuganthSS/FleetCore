import { Package, Clock, Truck, CheckCircle2, AlertTriangle, ArrowUpRight } from 'lucide-react';
import type { ShipmentStatus } from '@/types/shipment';

interface ShipmentKPIData {
  total: number;
  pending: number;
  dispatched: number;
  inTransit: number;
...
interface ShipmentKPICardsProps {
  data: ShipmentKPIData;
  activeStatusFilter: string;
  onStatusFilterChange: (status: string) => void;
}

export const ShipmentKPICards: React.FC<ShipmentKPICardsProps> = ({
  data,
  activeStatusFilter,
  ...
  const cards: Array<{
    ...
    },
    {
      ...
      count: data.failed + data.cancelled,
      icon: AlertTriangle,
      accentColor: 'text-destructive border-destructive/20',
      bgHover: 'hover:border-destructive/40',
    },
  ];
