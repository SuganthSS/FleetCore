...
export const FleetManagerReportsPage: React.FC = () => {
  // State management
  ...
  const { data: overview, isLoading, error, refetch, isFetching } = useQuery({
    queryKey: ['fleet-manager-reportsOverview', dateRange],
    queryFn: async () => {
  ...
  // Operational Report Templates (Excludes System Audit Log Template Modification)
  const reportTemplates: ReportTemplate[] = useMemo(
    () => [
      ...
      },
      {
        id: 'tpl-2',
        category: 'FLEET',
        title: 'Fleet Asset Status & Vehicle Assignment',
        description: 'Operational inventory status, active dispatches, and vehicle availability breakdown.',
        format: 'EXCEL',
        estimatedTime: '3s',
      },
      {
  ...
  // Operational Report History State
  ...
  // Operational Scheduled Reports State
  const [scheduledReports, setScheduledReports] = useState<ScheduledReport[]>([
    {
      id: 'SCH-101',
      reportName: 'Weekly Operational Fleet Availability Summary',
      frequency: 'WEEKLY',
      recipients: ['dispatch@fleetcore.com', 'ops@fleetcore.com'],
  ...
  const handleGenerateTemplate = (template: ReportTemplate) => {
    ...
    setSuccessMessage(`Report "${template.title}" generated and downloaded.`);
    clearAlertLater();
  };
  ...
  const handleBuilderGenerate = (config: ReportBuilderFormValues) => {
    ...
    setHistoryRecords((prev) => [newRecord, ...prev]);
    setSuccessMessage(`Custom Operational Report "${config.title}" generated.`);
    clearAlertLater();
  };
  ...
  const handleExportAll = () => {
    const content =
      ...
      `"All Fleet Operational Reports","${reportTemplates.length}","${new Date().toISOString()}"\n`;

    downloadFileClientSide(`fleet_reports_bundle_${Date.now()}.csv`, content);
    setSuccessMessage('Consolidated operational reports bundle exported.');
    clearAlertLater();
  };
  ...
  const handleToggleSchedule = (id: string) => {
    ...
    setSuccessMessage('Scheduled report status updated.');
    clearAlertLater();
  };
  ...
  const handleHistoryDelete = (id: string) => {
    ...
    setSuccessMessage('Report history entry removed.');
    clearAlertLater();
  };
  ...
  if (error) {
    return (
      <ReportErrorState
        title="Failed to Load Fleet Reports"
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider text-left">
          Operational Report Category Explorer
        </h3>
        <ReportCategoryGrid
          selectedCategory={selectedCategory}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
            Operational Report Templates ({filteredTemplates.length})
          </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <ScheduledReportsCard
            ...
            onCreateSchedule={() => {
              setSuccessMessage('Scheduled report creation workflow initiated.');
              clearAlertLater();
            }}
          />
        </div>
...
export default FleetManagerReportsPage;
